...
function PasswordGate({ onUnlock }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#F8F9FB", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"system-ui, sans-serif" }}>
      <div style={{ background:"#fff", border:"1px solid #DDE3ED", borderRadius:16, padding:"40px 36px", width:"100%", maxWidth:400, textAlign:"center" }}>
        ...
        <div style={{ position:"relative", marginBottom:12 }}>
          <input
            ...
            style={{ width:"100%", fontFamily:"inherit", fontSize:14, padding:"11px 44px 11px 14px", borderRadius:8, border:"1.5px solid #DDE3ED", background:"#F8F9FB", color:"#111827", outline:"none", boxSizing:"border-box" }}
          />
...
const STATUSES = {
  new:{ label:"New", dot:"#9CA3AF" }, contacted:{ label:"Contacted", dot:"#2563EB" },
  audit:{ label:"Audit sent", dot:"#B45309" }, talking:{ label:"In talks", dot:"#047857" },
  won:{ label:"Won", dot:"#047857" }, lost:{ label:"Passed", dot:"#B91C1C" },
};
const WEB_OPTS = [
...
function buildOpeningLine(svc, lead) {
  ...
  const lines = {
    ...
    cleaning: `"Most ${ind} businesses are working off inconsistent data and do not realize it. I found some patterns - mind if I share what I found?"`,
    etl:      `"I noticed ${n} likely has data in multiple systems that do not talk. I can connect them automatically - worth 15 minutes?"`,
    ...
    api:      `"I can connect ${n}'s systems so data flows automatically - no manual exports. I put together an integration map. Worth a look?"`,
...
function buildAuditHook(svc) {
  const hooks = {
    website:  "Build a 1-page homepage mockup with their branding. Send it cold - show, do not tell.",
...
function buildPitchPlan(lead) {
  ...
  if (hasWebsite==="none") return [
    { tier:"LEAD WITH",tcls:"lead",...SERVICES.website,        why:"No web presence - lead with the website build. Do not mention data yet. Establish trust first.",ol:buildOpeningLine("website",lead),     ah:buildAuditHook("website") },
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:10 }}>
      ...
      <div style={{ display:"flex", gap:5 }}>
        {[1,2,3,4,5].map(v => (
          <button key={v} onClick={() => onChange(v)} style={{ width:28, height:28, borderRadius:5, cursor:"pointer", fontFamily:mono, fontWeight:600, fontSize:11, display:"flex", alignItems:"center", justifyContent:"center", border:"1.5px solid "+(value>=v?color:C.border), background:value>=v?color+"18":C.surface2, color:value>=v?color:C.ink3 }}>{v}</button>
        ))}
      </div>
    </div>
  );
...
export default function App() {
  ...
  const [form,      setForm]      = useState({ name:"", industry:"", city:"", address:"", phone:"", website:"", hasWebsite:"none", size:"medium", dataSignals:3, techStack:3, reachability:3, proximity:3, webData:3, notes:"" });

  useEffect(() => { try { localStorage.setItem(STORAGE_KEY,JSON.stringify(leads)); } catch {} }, [leads]);

  const saveKey = useCallback(k => { setApiKey(k); try { localStorage.setItem(STORAGE_KEY+"-key",k); } catch {} }, []);
  ...
  const upd = useCallback((id,ch) => setLeads(p=>p.map(l=>l.id===id?{...l,...ch}:l)), []);
  const add = useCallback(data => setLeads(p=>[...p,{...data,id:Date.now(),status:"new",auditDone:false}]), []);
  ...
  const stats = { total:scored.length, hot:scored.filter(l=>l.score>=75).length, noWeb:scored.filter(l=>l.hasWebsite==="none").length, won:scored.filter(l=>l.status==="won").length };
    ...
    <div style={{ fontFamily:sans, background:C.page, minHeight:"100vh", color:C.ink, fontSize:14 }}>
      ...
      <header style={{ height:60, background:C.surface, borderBottom:"1px solid "+C.border, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 24px", gap:16, position:"sticky", top:0, zIndex:20 }}>
        ...
        <div style={{ display:"flex", gap:8, flexShrink:0 }}>
          {[[stats.total,"Leads",C.ink,C.surface,C.border],[stats.hot,"Hot leads",BRAND.primary,BRAND.bg,BRAND.border],[stats.noWeb,"No website",C.amber,C.amberBg,C.amberBdr],[stats.won,"Won",C.green,C.greenBg,C.greenBdr]].map(([v,l,col,bg2,bdr]) => (
            <div key={l} style={{ padding:"7px 16px", borderRadius:8, textAlign:"center", border:"1px solid "+bdr, background:bg2, minWidth:72 }}>
      ...
      {tab==="live" && (
        <div style={{ maxWidth:860, margin:"0 auto", padding:"32px 28px" }}>
          ...
          <p style={{ fontSize:14, color:C.ink3, lineHeight:1.7, marginBottom:28 }}>Search Google Maps in real time. Requires a free Places API key - <a href="https://console.cloud.google.com" target="_blank" rel="noreferrer" style={{ color:BRAND.primary, textDecoration:"none" }}>console.cloud.google.com</a>. Free $200/mo credit covers ~5,000 searches.</p>
          <div style={{ ...sc, marginBottom:20 }}>
            <div style={sl}>Google Places API key</div>
            ...
            <div style={{ fontSize:12, color:C.ink4, marginTop:8 }}>Saved locally in your browser only.</div>
          </div>
          ...
          {results.length>0 && (
            <>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                ...
                <button onClick={()=>{results.forEach(r=>add({...r,_id:undefined}));setResults([]);setTab("pipeline");}} style={{ ...bp, background:C.green, padding:"8px 20px", fontSize:13 }}>Import all ({results.length})</button>
              </div>
      ...
      {tab==="add" && (
        <div style={{ maxWidth:660, margin:"0 auto", padding:"32px 28px" }}>
          <h2 style={{ fontSize:20, fontWeight:700, color:C.ink, marginBottom:24 }}>Add lead manually</h2>
          <div style={{ display:"grid", gridTemplateColumns:"minmax(0,1fr) minmax(0,1fr)", gap:16, marginBottom:18 }}>
            {[["name","Business name",true],["industry","Industry",false],["address","Street address",true],["city","City / area",false],["phone","Phone",false],["website","Website",false]].map(([k,l,full])=>(
              <div key={k} style={{ gridColumn:full?"1/3":"auto" }}>
                ...
                <input value={form[k]} onChange={e=>setForm(p=>({...p,[k]:e.target.value}))} style={fi}/>
              </div>
            ))}
          </div>
          <div style={{ marginBottom:18 }}>
            ...
            <div style={{ display:"flex", gap:8 }}>
              {WEB_OPTS.map(o=>{
                ...
                return <button key={o.val} onClick={()=>setForm(p=>({...p,hasWebsite:o.val}))} style={{ flex:1, padding:"12px 8px", borderRadius:10, cursor:"pointer", textAlign:"center", fontFamily:sans, border:"1.5px solid "+(active?o.bdrColor:C.border), background:active?o.bgColor:C.surface2 }}><div style={{ fontSize:18, marginBottom:5 }}>{o.emoji}</div><div style={{ fontSize:13, fontWeight:600, color:active?o.txtColor:C.ink2 }}>{o.label}</div></button>;
              })}
            </div>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"minmax(0,1fr) minmax(0,1fr)", gap:16, marginBottom:18 }}>
            {SIGNALS.map(sig=>(
              <div key={sig.k}>
                ...
                <div style={{ display:"flex", gap:5 }}>
                  {[1,2,3,4,5].map(v=><button key={v} onClick={()=>setForm(p=>({...p,[sig.k]:v}))} style={{ flex:1, height:34, borderRadius:6, cursor:"pointer", fontFamily:mono, fontWeight:700, fontSize:12, border:"1.5px solid "+(form[sig.k]>=v?sig.color:C.border), background:form[sig.k]>=v?sig.color+"18":C.surface2, color:form[sig.k]>=v?sig.color:C.ink3 }}>{v}</button>)}
                </div>
              </div>
            ))}
            <div>
              ...
              <div style={{ display:"flex", gap:6 }}>
                {["small","medium","large"].map(v=><button key={v} onClick={()=>setForm(p=>({...p,size:v}))} style={{ flex:1, padding:"9px", borderRadius:7, cursor:"pointer", fontFamily:sans, fontSize:12, fontWeight:600, border:"1.5px solid "+(form.size===v?BRAND.border:C.border), background:form.size===v?BRAND.bg:C.surface2, color:form.size===v?BRAND.primary:C.ink2, textTransform:"capitalize" }}>{v}</button>)}
              </div>
            </div>
          </div>
          <div style={{ marginBottom:20 }}>
            ...
            <textarea value={form.notes} onChange={e=>setForm(p=>({...p,notes:e.target.value}))} rows={3} placeholder="Pain points, context, key contacts..." style={{ ...fi, resize:"none", lineHeight:1.65 }}/>
          </div>
          <div style={{ display:"flex", gap:10 }}>
            <button onClick={()=>{add(form);setForm(blankForm);setTab("pipeline");}} style={bp}>Add to pipeline</button>
            <button onClick={()=>setTab("pipeline")} style={bgh}>Cancel</button>
          </div>
        </div>
      )}
      ...
      {tab==="import" && (
        <div style={{ maxWidth:740, margin:"0 auto", padding:"32px 28px" }}>
          ...
          <p style={{ fontSize:14, color:C.ink3, lineHeight:1.7, marginBottom:28 }}>Export from <strong style={{ color:C.ink }}>Outscraper</strong> (free 500 records/month at outscraper.com) and paste below. Column names are auto-detected.</p>
          ...
          <textarea value={csvRaw} onChange={e=>setCsvRaw(e.target.value)} rows={12} placeholder={"name,phone,website,address,city,category\nForney Animal Hospital,(972) 564-3400,forneyanimal.com,1103 S Bois d'Arc St,Forney TX,Veterinarian"} style={{ ...fi, resize:"vertical", fontFamily:mono, fontSize:12, lineHeight:1.7, marginBottom:20 }}/>
          <div style={{ display:"flex", gap:10 }}>
            <button onClick={importCsv} style={bp}>Import leads</button>
            <button onClick={()=>setTab("pipeline")} style={bgh}>Cancel</button>
          </div>
        </div>
      )}
          ...
          <div style={{ width:sel?"44%":"100%", transition:"width .2s", overflowY:"auto", borderRight:"1px solid "+C.border, background:C.page }}>
            ...
            <div style={{ padding:12, display:"flex", flexDirection:"column", gap:6 }}>
              ...
              {visible.map((l,i)=>{
                ...
                const plan=buildPitchPlan(l); const wp=webPill(l.hasWebsite);
                const stripBdr=TIER_BORDERS[plan[0].tier];
                return (
                  <div key={l.id} onClick={()=>{setSelId(isAct?null:l.id);setPitchTab(0);}} style={{ background:isAct?BRAND.bg:C.surface, border:"1.5px solid "+(isAct?BRAND.primary:C.border), borderRadius:12, padding:"14px 16px", cursor:"pointer" }}>
                ...
                <div style={{ padding:"24px 26px" }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:22, gap:16 }}>
                    <div style={{ minWidth:0 }}>
                      ...
                      <div style={{ fontSize:20, fontWeight:700, color:C.ink, marginBottom:4, lineHeight:1.2 }}>{sel.name}</div>
                      <div style={{ fontSize:13, color:C.ink3, marginBottom:6 }}>{sel.industry}{sel.city?" - "+sel.city:""}</div>
                      {sel.address && <a href={"https://www.google.com/maps/search/?api=1&query="+encodeURIComponent(sel.address)} target="_blank" rel="noreferrer" style={{ display:"inline-flex", alignItems:"center", gap:4, fontSize:12, color:BRAND.primary, textDecoration:"none", marginBottom:5 }}><svg width="10" height="13" viewBox="0 0 10 13" fill="none"><path d="M5 0C2.79 0 1 1.79 1 4c0 3 4 9 4 9s4-6 4-9c0-2.21-1.79-4-4-4zm0 5.5A1.5 1.5 0 115 2.5 1.5 1.5 0 015 5.5z" fill="currentColor"/></svg>{sel.address}</a>}
                      <div style={{ display:"flex", gap:16, flexWrap:"wrap" }}>
                        {sel.phone && <span style={{ fontSize:12, color:C.ink3 }}>Phone: {sel.phone}</span>}
                        {sel.website ? <a href={"https://"+sel.website.replace(/^https?:\/\//,"")} target="_blank" rel="noreferrer" style={{ fontSize:12, color:BRAND.primary, textDecoration:"none" }}>Website: {sel.website}</a> : <span style={{ fontSize:12, color:C.red, fontWeight:600 }}>No website - web dev opportunity</span>}
                      </div>
                    </div>
                    <button onClick={()=>setSelId(null)} style={{ ...bgh, padding:"7px 14px", fontSize:13, flexShrink:0 }}>Close</button>
                  </div>
                  ...
                  <div style={sc}>
                    ...
                    <div style={{ display:"flex", gap:8, marginBottom:14 }}>
                      {pitch.map((pt,i)=>{
                        const ta=TIER_ACTIVE[pt.tcls]; const isActive=i===pitchTab;
                        return <div key={i} onClick={()=>setPitchTab(i)} style={{ flex:1, padding:"12px 8px", borderRadius:10, cursor:"pointer", textAlign:"center", border:"1.5px solid "+(isActive?ta.border:C.border), background:isActive?ta.bg:C.surface2 }}><div style={{ fontSize:16, marginBottom:4 }}>{pt.icon}</div><div style={{ fontFamily:sans, fontSize:9, fontWeight:700, letterSpacing:".6px", textTransform:"uppercase", color:isActive?ta.color:C.ink3, marginBottom:3 }}>{pt.tier}</div><div style={{ fontFamily:sans, fontSize:12, fontWeight:600, color:isActive?ta.color:C.ink2 }}>{pt.label}</div></div>;
                      })}
                    </div>
                    <div style={{ background:C.surface2, borderRadius:9, padding:16 }}>
                      ...
                      <div style={{ marginBottom:12, paddingLeft:14, borderLeft:"3px solid "+TIER_BORDERS[p.tier] }}>
                  ...
                  <div style={sc}>
                    <div style={sl}>Pipeline status</div>
                    <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginBottom:14 }}>
                      {Object.entries(STATUSES).map(([k,v])=>{
                        const isOn=sel.status===k;
                        const cm={ new:[C.surface2,C.border2,C.ink], contacted:[BRAND.bg,BRAND.border,BRAND.primary], audit:[C.amberBg,C.amberBdr,C.amber], talking:[C.greenBg,C.greenBdr,C.green], won:[C.greenBg,C.greenBdr,C.green], lost:[C.redBg,C.redBdr,C.red] };
                        const [bg2,bdr2,txt]=cm[k]||[C.surface2,C.border2,C.ink];
                        return <button key={k} onClick={()=>upd(sel.id,{status:k})} style={{ fontFamily:sans, fontSize:12, fontWeight:500, cursor:"pointer", borderRadius:6, padding:"6px 13px", border:"1.5px solid "+(isOn?bdr2:C.border), background:isOn?bg2:"transparent", color:isOn?txt:C.ink2 }}>{v.label}</button>;
                      })}
                    </div>
                  ...
                  <div style={sc}>
                    <div style={sl}>Website presence <span style={{ textTransform:"none", letterSpacing:0, fontSize:11, fontWeight:400, color:C.ink4 }}>- updates pitch strategy above</span></div>
                    <div style={{ display:"flex", gap:8, marginBottom:18 }}>
                      {WEB_OPTS.map(o=>{
                        const active=sel.hasWebsite===o.val;
                        return <button key={o.val} onClick={()=>{upd(sel.id,{hasWebsite:o.val});setPitchTab(0);}} style={{ flex:1, padding:"12px 8px", borderRadius:10, cursor:"pointer", textAlign:"center", fontFamily:sans, border:"1.5px solid "+(active?o.bdrColor:C.border), background:active?o.bgColor:C.surface2 }}><div style={{ fontSize:16, marginBottom:4 }}>{o.emoji}</div><div style={{ fontSize:12, fontWeight:600, color:active?o.txtColor:C.ink2 }}>{o.label}</div></button>;
                      })}
                    </div>
                    <div style={sl}>Lead signals</div>
                    {SIGNALS.map(sig=><SignalRow key={sig.k} label={sig.label} color={sig.color} value={sel[sig.k]} onChange={v=>upd(sel.id,{[sig.k]:v})}/>)}
                    <div style={{ display:"flex", gap:6, marginTop:12 }}>
                      {["small","medium","large"].map(v=><button key={v} onClick={()=>upd(sel.id,{size:v})} style={{ flex:1, padding:"9px", borderRadius:7, cursor:"pointer", fontFamily:sans, fontSize:12, fontWeight:600, border:"1.5px solid "+(sel.size===v?BRAND.border:C.border), background:sel.size===v?BRAND.bg:C.surface2, color:sel.size===v?BRAND.primary:C.ink2, textTransform:"capitalize" }}>{v}</button>)}
                    </div>
                  </div>

                  <div style={sc}>
                    <div style={sl}>Notes</div>
                    <textarea value={sel.notes} onChange={e=>upd(sel.id,{notes:e.target.value})} rows={4} placeholder="Pain points, decision maker name, audit findings, pricing notes..." style={{ fontFamily:sans, fontSize:13, background:C.surface2, border:"1.5px solid "+C.border, borderRadius:6, padding:"11px 13px", color:C.ink, outline:"none", width:"100%", resize:"vertical", lineHeight:1.65, boxSizing:"border-box" }}/>
                  </div>
                </div>
              </div>
            );
