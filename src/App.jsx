  ...
  return (
    <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:10 }}>
      ...
      <div style={{ display:"flex", gap:5 }}>
        {[1,2,3,4,5].map(v => (
          <button key={v} onClick={() => onChange(v)} style={{ width:28, height:28, borderRadius:5, cursor:"pointer", fontFamily:mono, fontWeight:600, fontSize:11, display:"flex", alignItems:"center", justifyContent:"center", border:`1.5px solid ${value >= v ? color : C.border}`, background:value >= v ? color+"18" : C.surface2, color:value >= v ? color : C.ink3, transition:"all .12s" }}>{v}</button>
        ))}
      </div>
    </div>
  );
...
export default function App() {
  ...
  const bg = { fontFamily:sans, fontSize:14, fontWeight:500, cursor:"pointer", borderRadius:6, padding:"11px 26px", border:`1.5px solid ${C.border}`, background:"transparent", color:C.ink2 };
      ...
      {tab === "live" && (
        <div style={{ maxWidth:860, margin:"0 auto", padding:"32px 28px" }}>
          ...
          <div style={{ display:"flex", gap:10, marginBottom:12 }}>
            ...
            <button onClick={runSearch} disabled={searching} style={{ ...bp, flexShrink:0, minWidth:110, opacity:searching?.7:1 }}>{searching?"Searching...":"Search"}</button>
          </div>
      ...
      {tab === "add" && (
        <div style={{ maxWidth:660, margin:"0 auto", padding:"32px 28px" }}>
          ...
          <div style={{ marginBottom:18 }}>
            ...
            <div style={{ display:"flex", gap:8 }}>
              {WEB_OPTS.map(o => {
                ...
                return <button key={o.val} onClick={() => setForm(p=>({...p,hasWebsite:o.val}))} style={{ flex:1, padding:"12px 8px", borderRadius:10, cursor:"pointer", textAlign:"center", fontFamily:sans, border:`1.5px solid ${active?o.bdrColor:C.border}`, background:active?o.bgColor:C.surface2, transition:"all .15s" }}><div style={{ fontSize:18, marginBottom:5 }}>{o.emoji}</div><div style={{ fontSize:13, fontWeight:600, color:active?o.txtColor:C.ink2 }}>{o.label}</div></button>;
              })}
            </div>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"minmax(0,1fr) minmax(0,1fr)", gap:16, marginBottom:18 }}>
            {SIGNALS.map(sig => (
              <div key={sig.k}>
                ...
                <div style={{ display:"flex", gap:5 }}>
                  {[1,2,3,4,5].map(v => <button key={v} onClick={() => setForm(p=>({...p,[sig.k]:v}))} style={{ flex:1, height:34, borderRadius:6, cursor:"pointer", fontFamily:mono, fontWeight:700, fontSize:12, border:`1.5px solid ${form[sig.k]>=v?sig.color:C.border}`, background:form[sig.k]>=v?sig.color+"18":C.surface2, color:form[sig.k]>=v?sig.color:C.ink3, transition:"all .12s" }}>{v}</button>)}
                </div>
              </div>
            ))}
            <div>
          ...
          <div style={{ display:"flex", gap:10 }}>
            ...
            <button onClick={() => setTab("pipeline")} style={bg}>Cancel</button>
          </div>
        </div>
      )}
          ...
          <div style={{ width:sel?"44%":"100%", transition:"width .2s", overflowY:"auto", borderRight:`1px solid ${C.border}`, background:C.page }}>
            ...
            <div style={{ padding:12, display:"flex", flexDirection:"column", gap:6 }}>
              ...
              {visible.map((l, i) => {
                ...
                return (
                  <div key={l.id} onClick={() => { setSelId(isAct?null:l.id); setPitchTab(0); }} style={{ background:isAct?BRAND.bg:C.surface, border:`1.5px solid ${isAct?BRAND.primary:C.border}`, borderRadius:12, padding:"14px 16px", cursor:"pointer", transition:"all .15s" }}>
              ...
              <div style={{ width:"56%", overflowY:"auto", background:C.surface }}>
                <div style={{ padding:"24px 26px" }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:22, gap:16 }}>
                    ...
                    <button onClick={() => setSelId(null)} style={{ ...bg, padding:"7px 14px", fontSize:13, flexShrink:0 }}>Close</button>
                  </div>
                  ...
                  <div style={sc}>
                    ...
                    <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginBottom:14 }}>
                      {Object.entries(STATUSES).map(([k,v]) => {
                        ...
                        const [bg2,bdr2,txt] = colorMap[k]||[C.surface2,C.border2,C.ink];
                        return <button key={k} onClick={() => upd(sel.id,{status:k})} style={{ fontFamily:sans, fontSize:12, fontWeight:500, cursor:"pointer", borderRadius:6, padding:"6px 13px", border:`1.5px solid ${isOn?bdr2:C.border}`, background:isOn?bg2:"transparent", color:isOn?txt:C.ink2, transition:"all .12s" }}>{v.label}</button>;
                      })}
                    </div>
                  ...
                  <div style={sc}>
                    ...
                    <div style={{ display:"flex", gap:8, marginBottom:18 }}>
                      {WEB_OPTS.map(o => {
                        ...
                        return <button key={o.val} onClick={() => { upd(sel.id,{hasWebsite:o.val}); setPitchTab(0); }} style={{ flex:1, padding:"12px 8px", borderRadius:10, cursor:"pointer", textAlign:"center", fontFamily:sans, border:`1.5px solid ${active?o.bdrColor:C.border}`, background:active?o.bgColor:C.surface2, transition:"all .15s" }}><div style={{ fontSize:16, marginBottom:4 }}>{o.emoji}</div><div style={{ fontSize:12, fontWeight:600, color:active?o.txtColor:C.ink2 }}>{o.label}</div></button>;
                      })}
                    </div>
